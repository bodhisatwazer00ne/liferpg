...
export async function directSignUp(
  ...
  if (existing) {
    ...
    try {
      await setDoc(doc(db, 'trainerAccounts', accountId), sanitizeForFirestore(updatedAccount), { merge: true });
    } catch (err) {
  ...
  // Completely new user: use deterministic UID derived from account ID to ensure persistent identification
  const salt = generateSalt();
  const passwordHash = await hashPassword(pass, salt);
  const uid = accountId;
  ...
  try {
    await setDoc(
      ...
      sanitizeForFirestore({
        uid: accountId, // Canonical UID always matches accountId for 1:1 email mapping
        googleUid: googleUid,
        email: cleanEmail,
